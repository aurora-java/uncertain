...
public class DirectoryConfig extends DynamicObject {
     ...
     * @param path_with_tag A String containing path tag, such as "${logDirectory}/dailylog/2012/"
     * @return
     */
    public String translateRealPath( String path_with_tag ){
        QuickTagParser   parser = new QuickTagParser();
        MapHandle     handle = new MapHandle(this.getObjectContext(), Character.LOWERCASE_LETTER);
     ...
     * @throws IOException
     */
    public void loadConfigProperties( File property_file)
        throws IOException
    {
    ...
    public void validateConfig(){
        ...
        while(it.hasNext()){
            ...
            Object value = entry.getValue();
            if(value==null)
                throw BuiltinExceptionFactory.createInvalidPathException(object_context.asLocatable(), "("+entry.getKey().toString()+"=null)");
            String path = value.toString();
            File file = new File(path);
            if(!file.exists() || !file.isDirectory())
                throw BuiltinExceptionFactory.createInvalidPathException(object_context.asLocatable(), path);
        }        
    }
